...
array at 1.0 microsec per pixel and subarray readout.    -ARC

Now the reset array routine resets the array on a line by line
basis.  This means that the array resets in <1ms.        -KRS 3/7/2002
   **
...
DELAY    EQU $480000                ; Delay for clocking ops (20 ns, 160 if MSB set)
...
SHIFT_ODD_ROW
   ...
   DC CLK2+H_READ+L_PIXEL+H_LSYNC+L_LINE+H_FSYNC+H_RST+DELAY
   DC CLK2+H_READ+L_PIXEL+H_LSYNC+H_LINE+H_FSYNC+H_RST+DELAY
   DC CLK2+H_READ+H_PIXEL+H_LSYNC+H_LINE+H_FSYNC+H_RST
...
SHIFT_EVEN_ROW
   ...
   DC CLK2+H_READ+L_PIXEL+H_LSYNC+H_LINE+H_FSYNC+H_RST+DELAY
   DC CLK2+H_READ+L_PIXEL+H_LSYNC+L_LINE+H_FSYNC+H_RST+DELAY
   DC CLK2+H_READ+H_PIXEL+H_LSYNC+L_LINE+H_FSYNC+H_RST
...
; Check for overflow in the EEPROM case
   IF    @SCP("DOWNLOAD","EEPROM")
      IF    @CVS(N,@LCV(L))>(APL_NUM+1)*N_W_APL
         WARN  'EEPROM overflow!'   ; Make sure next application will not be overwritten
      ENDIF
   ENDIF

   ENDSEC                           ; End of section HINZLR
   END                              ; End of program
